...
func update(m, l):
	...
	func make_bruteforce_match(i, j):
		return {
			'pattern': 'bruteforce',
...
func estimate_guesses(match, password):
...
func repeat_guesses(match):
	return match['base_guesses'] * Decimal(match['repeat_count'])


func sequence_guesses(match):
...
const KEYBOARD_AVERAGE_DEGREE = calc_average_degree(AdjacencyGraphs.ADJACENCY_GRAPHS['qwerty'])
# slightly different for keypad/mac keypad, but close enough
const KEYPAD_AVERAGE_DEGREE = calc_average_degree(AdjacencyGraphs.ADJACENCY_GRAPHS['keypad'])

const KEYBOARD_STARTING_POSITIONS = len(AdjacencyGraphs.ADJACENCY_GRAPHS['qwerty'].keys())
const KEYPAD_STARTING_POSITIONS = len(AdjacencyGraphs.ADJACENCY_GRAPHS['keypad'].keys())


func spatial_guesses(match):
...
func uppercase_variations(match):
	word = match['token']
